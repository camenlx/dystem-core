...
#if defined(NDEBUG)
#error "Dystem cannot be compiled without assertions."
#endif
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    if (!CheckTransaction(tx, state))
        return error("AcceptToMemoryPool: : CheckTransaction has thrown an error");
...
{    
    ...
    if (nHeight < Params().LAST_POW_BLOCK())                // < BLOCK: 200         : 800,000 Premine   TOTAL 800,000         
        nSubsidy = 4000 * COIN;                         
    else if (nHeight <= 6100)                               // < BLOCK: 6100        : 3050              TOTAL 803050
        nSubsidy = 0.5 * COIN;
    else if (nHeight > 6100 && nHeight <= 36100)            // < Block: 36100       : 30000             TOTAL 833050
         nSubsidy = 1 * COIN;
    else if (nHeight > 36100 && nHeight <= 100100)          // < Block: 100100      : 160000            TOTAL 993050              
         nSubsidy = 2.5 * COIN;
     else if (nHeight > 100100 && nHeight <= 120100)        // < Block: 120100      : 500000            TOTAL 1493050             
        nSubsidy = 25 * COIN;
    else if (nHeight > 120100 && nHeight <= 220100)         // < Block: 220100      : 1500000           TOTAL 2993050
        nSubsidy = 15 * COIN;
    else if (nHeight > 220100 && nHeight <= 320100)         // < Block: 320100      : 1000000           TOTAL 3993050
        nSubsidy = 10 * COIN;
    else if (nHeight > 320100 && nHeight <= 1320100)        // < Block: 1320100     : 8000000           TOTAL 11993050
        nSubsidy = 8 * COIN;
    else if (nHeight > 1320100 && nHeight <= 2320100)       // < Block: 1200100     : 6000000           TOTAL 17993050
        nSubsidy = 6 * COIN;
    else if (nHeight > 2320100 && nHeight <= 2920100)       // < Block: 2920100     : 2400000           TOTAL 20393050
        nSubsidy = 4 * COIN;
    else
        nSubsidy = 2 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    ret = blockValue * 0.825; // 85% of block reward
...
void static InvalidBlockFound(CBlockIndex* pindex, const CValidationState& state)
{
    ...
    if (state.IsInvalid(nDoS)) {
        ...
        if (it != mapBlockSource.end() && State(it->second)) {
            ...
            if (nDoS > 0)
                Misbehaving(it->second, nDoS, std::string("main::InvalidBlockFound"));
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        if (pfrom->nVersion != 0) {
            ...
            Misbehaving(pfrom->GetId(), 1, std::string("main::ProcessMessage::ln4454"));
            return false;
        }
        ...
        Misbehaving(pfrom->GetId(), 1, std::string("main::ProcessMessage::ln4559"));
        return false;
    }
        ...
        if (pfrom->nVersion < CADDR_TIME_VERSION && addrman.size() > 1000)
            return true;
        if (vAddr.size() > 1000) {
            Misbehaving(pfrom->GetId(), 20, std::string("main::ProcessMessage::ln4583"));
        ...
        if (vInv.size() > MAX_INV_SZ) {
            Misbehaving(pfrom->GetId(), 20, std::string("main::ProcessMessage::ln4641"));
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++) {
            ...
            if (pfrom->nSendSize > (SendBufferSize() * 2)) {
                Misbehaving(pfrom->GetId(), 50, std::string("main::ProcessMessage::ln4675"));
        ...
        vRecv >> vInv;
        if (vInv.size() > MAX_INV_SZ) {
            Misbehaving(pfrom->GetId(), 20, std::string("main::ProcessMessage::ln4689"));
                ...
                for(set<uint256>::iterator mi = itByPrev->second.begin();
                    ...
                    if(AcceptToMemoryPool(mempool, stateDummy, orphanTx, true, &fMissingInputs2)) {
                        ...
                        if(stateDummy.IsInvalid(nDos) && nDos > 0) {
                            ...
                            Misbehaving(fromPeer, nDos, std::string("main::ProcessMessage::ln4834"));
        ...
        if (state.IsInvalid(nDoS)) {
            ...
            if (nDoS > 0)
                Misbehaving(pfrom->GetId(), nDoS, std::string("main::ProcessMessage::ln4872"));
        }
    }
    ...
    else if (strCommand == "headers" && Params().HeadersFirstSyncingActive() && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (nCount > MAX_HEADERS_RESULTS) {
            Misbehaving(pfrom->GetId(), 20, std::string("main::ProcessMessage::ln4884"));
        ...
        BOOST_FOREACH (const CBlockHeader& header, headers) {
            ...
            if (pindexLast != NULL && header.hashPrevBlock != pindexLast->GetBlockHash()) {
                Misbehaving(pfrom->GetId(), 20, std::string("main::ProcessMessage::ln4903"));
            ...
            if (!AcceptBlockHeader((CBlock)header, state, &pindexLast)) {
                ...
                if (state.IsInvalid(nDoS)) {
                    if (nDoS > 0)
                        Misbehaving(pfrom->GetId(), nDoS, std::string("main::ProcessMessage::ln4914"));
        ...
        if (!mapBlockIndex.count(block.hashPrevBlock)) {
            ...
            if (!mapBlockIndex.count(block.GetHash())) {
                ...
                if(state.IsInvalid(nDoS)) {
                    ...
                    if(nDoS > 0) {
                        TRY_LOCK(cs_main, lockMain);
                        if(lockMain) Misbehaving(pfrom->GetId(), nDoS, std::string("main::ProcessMessage::ln4966"));
                    }
                }
    ...
    // getaddr message mitigates the attack.
        ...
        if (pfrom->setKnown.count(alertHash) == 0) {
            if (alert.ProcessAlert()) {
                ...
                Misbehaving(pfrom->GetId(), 10, std::string("main::ProcessMessage::ln5112"));
            }
        }
    }
        ...
        Misbehaving(pfrom->GetId(), 100, std::string("main::ProcessMessage::ln5122"));
    }
        ...
        if (!filter.IsWithinSizeConstraints())
            ...
            Misbehaving(pfrom->GetId(), 100, std::string("main::ProcessMessage::ln5131"));
        else {
            LOCK(pfrom->cs_filter);
        ...
        if (vData.size() > MAX_SCRIPT_ELEMENT_SIZE) {
            Misbehaving(pfrom->GetId(), 100, std::string("main::ProcessMessage::ln5149"));
        } else {
            LOCK(pfrom->cs_filter);
            if (pfrom->pfilter)
                pfrom->pfilter->insert(vData);
            else
                Misbehaving(pfrom->GetId(), 100, std::string("main::ProcessMessage::ln5155"));
        }
    }

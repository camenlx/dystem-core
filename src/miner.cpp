...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        LogPrint("net","CreateNewBlock(): total size %u\n", nBlockSize);
...
{
    LogPrint("net","%s\n", pblock->ToString());
    LogPrint("net","generated %s\n", FormatMoney(pblock->vtx[0].vout[0].nValue));

    // Found a solution
    {
        LOCK(cs_main);
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("Dyystem Miner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("DYSTEMMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("DYSTEMMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dystem-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        //Stake miner main
        if (fProofOfStake) {
            LogPrint("net","CPUMiner : proof-of-stake block found %s \n", pblock->GetHash().ToString().c_str());

            if (!pblock->SignBlock(*pwallet)) {
                LogPrint("net","BitcoinMiner(): Signing new block failed \n");
                continue;
            }

            LogPrint("net","CPUMiner : proof-of-stake block was signed %s \n", pblock->GetHash().ToString().c_str());
        ...
        LogPrintf("Running DystemMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),

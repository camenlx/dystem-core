...
{   
    ...
    if (nHeight <= 6100)                            
        nSubsidy = 0.525 * COIN;
    else if (nHeight > 6100 && nHeight <= 36100)    
        nSubsidy = 1.07 * COIN;
    else if (nHeight > 36100 && nHeight <= 100100)
        nSubsidy = 2.55 * COIN;
    else if (nHeight > 100100 && nHeight <= 120100)
        nSubsidy = 25.9 * COIN;
    else if (nHeight > 120100 && nHeight <= 220100)
        nSubsidy = 15.25 * COIN;
    else if (nHeight > 220100 && nHeight <= 320100)
        nSubsidy = 10.53 * COIN;
    else if (nHeight > 320100 && nHeight <= 1320100)
        nSubsidy = 8.03 * COIN;
    else if (nHeight > 1320100 && nHeight <= 2320100)
        nSubsidy = 6.03 * COIN;
    else if (nHeight > 2320100 && nHeight <= 2920100)
        nSubsidy = 4.23 * COIN;
    else
        nSubsidy = 2.12 * COIN;
...
void CBudgetManager::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
        ...
        if (Params().NetworkID() == CBaseChainParams::MAIN) {
            if (nProp == 0) {
                if (pfrom->HasFulfilledRequest("mnvs")) {
                    ...
                    Misbehaving(pfrom->GetId(), 20, std::string("masternode-budget::ProcessMessage::ln 997"));
                    return;
                }
    ...
    if (strCommand == "mvote") { //Masternode Vote
        ...
        if (!vote.SignatureValid(true)) {
            ...
            if (masternodeSync.IsSynced()) Misbehaving(pfrom->GetId(), 20, std::string("masternode-budget::ProcessMessage::ln 1065"));
    ...
    if (strCommand == "fbvote") { //Finalized Budget Vote
        ...
        if (!vote.SignatureValid(true)) {
            ...
            if (masternodeSync.IsSynced()) Misbehaving(pfrom->GetId(), 20, std::string("masternode-budget::ProcessMessage::ln 1137"));
            // it could just be a non-synced masternode
            mnodeman.AskForMN(pfrom, vote.vin);
            return;
        }

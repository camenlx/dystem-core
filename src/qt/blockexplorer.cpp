...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::DTEM, nValue);
...
{
    std::string genesisblockhash = "0x00000f4fb42644a07735beea3647155995ab01cf49d05fdc082c08eb673433f9"; 
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (dystem.conf).");
            QMessageBox::warning(this, "Dystem Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}

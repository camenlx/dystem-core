...
void CMasternode::Check(bool forceCheck)
{
    ...
    if (!unitTest) {
        ...
        CTxOut vout = CTxOut((5000-0.01), masternodeSigner.collateralPubKey);
...
bool CMasternodeBroadcast::CheckDefaultPort(std::string strService, std::string& strErrorRet, std::string strContext)
{
    ...
    if (service.GetPort() <= (nDefaultPort -1) || service.GetPort() >= (nDefaultPort + 10) ) {
...
bool CMasternodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    if (!masternodeSigner.VerifyMessage(pubKeyCollateralAddress, sig, GetNewStrMessage(), errorMessage)
    		&& !masternodeSigner.VerifyMessage(pubKeyCollateralAddress, sig, GetOldStrMessage(), errorMessage))
    {
